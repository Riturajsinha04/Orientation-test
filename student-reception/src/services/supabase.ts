...
export const syncStudentToSupabaseDirect = async (tokenData: {
  ...
  try {
    ...
    if (existingData && existingData.length > 0) {
      ...
      console.log(`[Direct Supabase Sync] Updated existing token ${tokenStr} in Supabase.`);
    } else {
      ...
      if (error) {
        console.warn('[Direct Supabase Sync Notice]', error.message);
      } else {
        console.log(`[Direct Supabase Sync] Registered ${tokenData.studentName} (Token ${tokenStr}, ID: ${newId}) in Supabase database table 'tokens'.`);
      }
    }
  } catch (err) {
    console.error('[Direct Supabase Sync Exception]', err);
  }
};
