...
import { syncStudentToSupabaseDirect } from '../services/supabase';
import {
  Search,
  Ticket,
  CheckCircle2,
...
const COURSES = [
  { id: 'cs', title: 'B.Tech Computer Science & AI', code: 'CSE-AI', icon: '💻' },
  { id: 'it', title: 'B.Tech Information Technology', code: 'IT', icon: '⚡' },
  { id: 'ece', title: 'B.Tech Electronics & Comm', code: 'ECE', icon: '📡' },
  { id: 'mech', title: 'B.Tech Mechanical & Robotics', code: 'ME-ROB', icon: '🤖' },
  { id: 'ds', title: 'Data Science & Analytics', code: 'DSA', icon: '📊' },
  { id: 'cyber', title: 'Cyber Security & Cloud', code: 'CSC', icon: '🔒' },
];
...
export const ReceptionPage: React.FC = () => {
  ...
  const [course, setCourse] = useState('B.Tech Computer Science & AI');
  ...
  const handleSubmitRegistration = async (allowDuplicate = false) => {
    ...
    try {
      ...
      if (res.success && res.token) {
        ...
        syncStudentToSupabaseDirect({
          token: res.token.token,
          studentName: res.token.studentName,
          mobile: res.token.mobile,
          course: res.token.course,
      ...
      <div className="bg-gradient-to-r from-slate-900 via-blue-950 to-slate-900 rounded-3xl p-6 sm:p-8 text-white shadow-xl border border-slate-800 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <h2 className="text-2xl sm:text-3xl font-black tracking-tight font-outfit text-white">
            RECEPTION TOKEN TERMINAL
          </h2>
          <p className="text-xs sm:text-sm text-slate-300 font-medium mt-1 max-w-xl">
            Register new students, assign academic program tokens, and provide ticket receipts for classroom orientation.
          </p>
        </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-900">
                    {step === 1 ? 'Step 1: Student Information' : 'Step 2: Select Academic Program'}
                  </h3>
                  <p className="text-xs text-slate-500 font-medium">
                    {step === 1 ? 'Enter candidate name and 10-digit phone number' : 'Assign program and generate live token'}
                  </p>
                </div>
              </div>

              <div className="flex items-center space-x-2">
                <span className={`w-3 h-3 rounded-full ${step === 1 ? 'bg-blue-600' : 'bg-emerald-500'}`}></span>
                <span className={`w-3 h-3 rounded-full ${step === 2 ? 'bg-blue-600' : 'bg-slate-200'}`}></span>
              </div>
            </div>
            ...
            {step === 1 && (
              <div className="space-y-6">
                ...
                <button
                  ...
                  className="w-full py-4 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white font-extrabold text-sm rounded-2xl shadow-md hover:shadow-lg transition-all flex items-center justify-center space-x-2"
                >
                  <span>PROCEED TO PROGRAM SELECTION</span>
                  <ChevronRight className="w-4 h-4" />
                </button>
              </div>
            )}

            {/* STEP 2: Program Selection & Generate Token */}
            {step === 2 && (
              <div className="space-y-6">
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <label className="block text-xs font-extrabold text-slate-700 uppercase tracking-wider">
                      Select Academic Program <span className="text-rose-500">*</span>
                    </label>
                    <span className="text-xs font-semibold text-blue-600">Candidate: {studentName}</span>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                    {COURSES.map((c) => {
                      const isSelected = course === c.title;
                      return (
                        <div
                          key={c.id}
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <span className="text-xs font-extrabold text-blue-700 bg-blue-100 border border-blue-200 px-2 py-0.5 rounded-md font-mono inline-block mb-1">
                              {c.code}
                            </span>
                ...
                <div className="flex items-center space-x-3 pt-2">
                  <button
                    type="button"
                    onClick={() => setStep(1)}
                    className="py-3.5 px-5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-2xl transition-all flex items-center space-x-2"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-black text-sm rounded-2xl shadow-lg hover:shadow-xl transition-all flex items-center justify-center space-x-2 disabled:opacity-50"
                  >
                    <Ticket className="w-5 h-5" />
